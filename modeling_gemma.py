...
class KVCache():
    ...
    def update(
        ...
        if len(self.key_cache)<=layer_idx:
            # if we never addded anyting to the KV-Cache of this layer, let's create it.
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self,x,position_ids,seq_len):
        ...
        with torch.autocast(device_type=device_type,enabled=False):
            ...
            # freqs: [Batch_Size, Head_Dim//2, Seq_Len, 1]@ [ Batch_Size, 1, Seq_Len] --> [Batch_Size, Seq_Len, Head_Dim//2]
            freqs= (inv_freq_expanded.float()@position_ids_expanded.float()).transpose(1,2)
            # emb: [Batch_Size, Head_Dim//2, Seq_Len]
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        # Apply the softmax
        # [Batch_Size, Num_Heads_Q, Seq_len_Q, Seq_ken_KV]
